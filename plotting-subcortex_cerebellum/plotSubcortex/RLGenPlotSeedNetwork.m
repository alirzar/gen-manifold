...
for i = 1:length(fileList)
    ...
    resSub = res(ismember(res.network, sub_labels.subcorticalLabels), :);
    ...
    roi_indices = cellfun(@(roi) label_map(roi), resSub.network);
    % Update the T values in the tvals table using the mapped indices
    rvals.r(roi_indices) = resSub.r;
    ...
    % colormap(getSeedNetCmap(512, 0, 1));
    colormap(cmap)
