...
%     % Define the red, blue and gray RGB values
%     red = [1, 0, 0];
%     blue = [0, 0, 1];
%     gray = [0.5, 0.5, 0.5];
% 
%     % Generate a colormap that transitions from blue to gray to red
%     m = 128;  % Number of colors in each half of the colormap
%     cmap1 = [linspace(blue(1), gray(1), m)', linspace(blue(2), gray(2), m)', linspace(blue(3), gray(3), m)'];
%     cmap2 = [linspace(gray(1), red(1), m)', linspace(gray(2), red(2), m)', linspace(gray(3), red(3), m)'];
% 
%     % Combine the two colormaps to create the final colormap
%     cmapFinal = [cmap1; cmap2];
% end
function cmapFinal = getRedBlueCmap()
    % Define the red, blue, and lighter gray RGB values
    red = [1, 0, 0];
    blue = [0, 0, 1];
    gray = [0.8, 0.8, 0.8];  % Lighter gray
