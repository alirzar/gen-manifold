function varargout=plotflatmap(data,varargin)
...
indx = ~isnan(OCOL); 
if (isscalar(alpha))
    COL(indx) = OCOL(indx).*((1-alpha)*ones(size(COL(indx)))+alpha*COL(indx)); % Set opacacy of underlay 
else 
    for i=1:3 
        ALPHA(:,:,i) = [alpha(C.faces(k(:),1),1) alpha(C.faces(k(:),2),1) alpha(C.faces(k(:),3),1)]';
    end
    COL(indx) = OCOL(indx).*((1-ALPHA).*ones(size(COL(indx)))+ALPHA.*COL(indx)); 
end
