...
from genman.utils import get_surfaces
...
def yeo_cmap(as_palette=False, networks=7):
    ...
    if networks == 17:
        ...
        cmap = {
            ...
            'LimbicA': (224, 248, 166),
            'LimbicB': (126, 135, 55),
            'Subcortex': (0, 0, 0),       # New color for 'Subcortex'
            'Cerebellum': (100, 100, 100)      # New color for 'Cerebellum'
            # 'Network1': (255, 128, 0),
            # 'Network2': (0, 128, 255),
        }
    else:
...
def plot_3d(x, y, z, ax=None, view_3d=(30, -120), **kwargs):
    ...
    **kwargs : dict, optional
    ...
    ax.set(xticks=np.arange(-2, 4), 
           ...
           zticks=np.arange(-2, 4))
    if view_3d is not None:
        ax.view_init(elev=view_3d[0], azim=view_3d[1])

    # ax.w_xaxis.set_pane_color((0.7155, 0.800, 0.8275))
    # ax.w_yaxis.set_pane_color((0.7155, 0.800, 0.8275))
    # ax.w_zaxis.set_pane_color((0.7155, 0.800, 0.8275))
...
def pairwise_stat_maps(data, prefix, layercbar=False, dorsal=True, posterior=True, vmax='auto', 
    ...
    for i in tvals.columns:
        ...
        #vmax = np.nanmax(tvals.values)
        #vmin = np.nanmax(tvals.values)
        #layer_params = dict(cmap=cmap, cbar=True, color_range=(-vmax, vmax))
        # row 
