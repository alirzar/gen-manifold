...
from genman.utils import get_surfaces, permute_map
...
def plot_correlations(data, out_dir):
    ...
    for i, m in enumerate(measures):
        ...
        scatter_kws = dict(s=4, alpha=1, color='.5',
                           clip_on=False)
        line_kws = dict(color='k', linewidth=1)
        fig, ax = plt.subplots(figsize=(2, 2))
        sns.scatterplot(data=data, x=m, y='distance', ax=ax, hue='network', 
                        s=6, alpha=1, clip_on=False, palette=cmap, legend=False)
        sns.regplot(x=m, y='distance', data=data, scatter=False, 
                          line_kws=line_kws, ci=None, ax=ax)
...
def main():
    ...
    fname = os.path.join(config.connect + '-centered', 
                         'reference_cmat.tsv')
    cmat = pd.read_table(fname, index_col=0)#.iloc[32:432, 32:432]
    data = compute_measures(cmat)
    ecc = pd.read_table(os.path.join(config.results, 'ref_ecc.tsv'), sep='\t')
    data = data.merge(ecc, on='roi')
    data_cereb = data.query('network not in ["Subcortex", "Cerebellum"]')
    out_dir = os.path.join(config.results, 'fc_correlations')
    corrs, nulls, spins = run_correlations(data_cereb, out_dir, overwrite=True)

    plot_maps(data_cereb, FIG_DIR)
    plot_correlations(data, FIG_DIR)
    data.to_csv(os.path.join(out_dir, 'fc_connectivity_measures.tsv'), sep='\t')
